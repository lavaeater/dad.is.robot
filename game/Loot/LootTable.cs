...
namespace rds
{
	...
	/// The Result implementation of the ILootTable interface uses the LootRandom class
	...
	public class LootTable : ILootTable
	{
		...
		public LootTable(IEnumerable<ILootObject> contents, int count, double probability)
		...
		public LootTable(IEnumerable<ILootObject> contents, int count, double probability, bool unique, bool always, bool enabled)
		{
			if (contents != null)
		...
		/// </summary>
		public IEnumerable<ILootObject> rdsContents
		{
			get { return mcontents; }
		}
		private List<ILootObject> mcontents = null;
		...
		{
			mcontents = new List<ILootObject>();
		}
		...
		public virtual void AddEntry(ILootObject entry)
		{
			mcontents.Add(entry);
			entry.Table = this;
		}
		...
		/// constructors of LootObject or if you implemented ILootObject directly in your class and you need
		/// to (re)apply a new probability at the moment you add it to a LootTable.
		/// NOTE: The probability given is written back to the given instance "entry".
		/// </summary>
		/// <param name="entry">The entry.</param>
		/// <param name="probability">The probability.</param>
		public virtual void AddEntry(ILootObject entry, double probability)
		{
			mcontents.Add(entry);
		...
		/// Use this signature if (for whatever reason) the base classes constructor does not support all
		/// constructors of LootObject or if you implemented ILootObject directly in your class and you need
		/// to (re)apply a new probability and flags at the moment you add it to a LootTable.
		...
		public virtual void AddEntry(ILootObject entry, double probability, bool unique, bool always, bool enabled)
		{
			mcontents.Add(entry);
			entry.Probability = probability;
			entry.Unique = unique;
		...
		/// </summary>
		/// <param name="entry">The entry.</param>
		public virtual void RemoveEntry(ILootObject entry)
		{
		...
		{
			ILootObject entry = mcontents[index];
			entry.Table = null;
		...
		#region RESULT
		private List<ILootObject> uniquedrops = new List<ILootObject>();

		private void AddToResult(List<ILootObject> rv, ILootObject o)
		{
			if (!o.Unique || !uniquedrops.Contains(o))
			{
				...
				if (!(o is LootNullValue))
				{
					if (o is ILootTable)
					{
						rv.AddRange(((ILootTable)o).rdsResult);
					}
					else
					{
						// INSTANCECHECK
						// Check if the object to add implements LootObjectCreator.
						...
						ILootObject adder = o;
						if (o is LootObjectCreator)
							adder = ((LootObjectCreator)o).CreateInstance();
		...
		/// </summary>
		public virtual IEnumerable<ILootObject> rdsResult
		{
			get
			{
				// The return value, a list of hit objects
				List<ILootObject> rv = new List<ILootObject>();
				...
				foreach (ILootObject o in mcontents)
				...
				foreach (ILootObject o in mcontents.Where(e => e.Always && e.Enabled))
					AddToResult(rv, o);
				...
				if (realdropcnt > 0)
				{
					for (int dropcount = 0; dropcount < realdropcnt; dropcount++)
					{
						...
						IEnumerable<ILootObject> dropables = mcontents.Where(e => e.Enabled && !e.Always);
						...
						foreach (ILootObject o in dropables)
						{
		...
		public string ToString(int indentationlevel)
		{
			...
			sb.AppendFormat(indent + "(LootTable){0} Entries:{1},Prob:{2},UAE:{3}{4}{5}{6}", 
